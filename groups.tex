...
$G/\Phi(G)\cong(\mathbb{F}_p)^d$, $d$ is the minimal number of generators.
...
\label{subsec:semidirect}

\textit{Definition.} $G=N\rtimes H$ where $N\trianglelefteq G$, $H\le G$, $NH=G$, $N\cap H=\{e\}$. Multiplication is determined by $\varphi:H\to\operatorname{Aut}(N)$:
$$(n_1,h_1)(n_2,h_2)=(n_1\,\varphi_{h_1}(n_2),\,h_1h_2).$$

\textit{Examples.} $D_n\cong C_n\rtimes C_2$ (inversion); classification of groups of order $pq$ (\ref{subsec:order-pq}) gives $\mathbb{Z}/q\rtimes\mathbb{Z}/p$ when $p\mid q-1$.

\textit{Recognition.} If $N\trianglelefteq G$, $G=NH$, $N\cap H=\{e\}$, then $G\cong N\rtimes H$.

\medskip

\subsection{Common techniques}
\label{subsec:group-techniques}
